...
# designer:shihua
# coder:shihua
# 这是一个SunShareClient数据服务sdk类，主要功能提供数据接口，主要技术requests,http和partial
"""
模块介绍
-------
...
from sunshareclient.client.cons import API_DICT



####### SunShareClient服务应用用户路由 #######################################
### 设计模式：                                                            ###
###     无                                                               ###
### 关键点：                                                              ###
### （1）requests                                                         ###
...
class DataAPI(object):
    ...
    __token_key = ''
    __token = ''
    __http_url = '127.0.0.1:8000' ### 暂时用不上，具体数据服务路由信息可在sunshareclient.client.cons中获得；后续如果有了固定数据服务地址可在此使用此私有属性固定住


    def __init__(self,token_key,token,timeout):
        '''
        属性功能：
    ...
    def query(self,dataapi,params):
        ...
        ### 参数字典中加入token_key和token,使用http发送get协议的时候不需要考虑参数顺序
...
def get_dataframe_from_request(url,timeout):
    ...
    if type(js) != list:
        ...
        ### 转换成dataframe
        df = pd.DataFrame(js)    
        return df ### 此处返回的dataframe在stdout中可能无法全部显示，可以导出在csv中查看或直接内存中操作，jupter中测试已通过
